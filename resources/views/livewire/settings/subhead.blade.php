<div class="container-xxl flex-grow-1 container-p-y">
                  ...
                  @forelse ($subs as $sub)
                    <tr wire:key='{{$sub->id}}'>
                        ...
                        <td class="text-center">
                          ...
                          <livewire:edit-anchor :record="$sub" :eventoption="$editevent" wire:key='{{time().$sub->id}}' > 
                           &nbsp;
                           
                            <a href="#" onclick="confirm('Are you sure you want to delete {{$sub->name}} ?') ? '' : event.stopImmediatePropagation()" wire:click='delete({{$sub->id}})'>
  ...
  <x-add-sub-head :title="$title" :edit=$edit :heads="$heads" :sub="$sub"/>
